...
class Pessoa: # definido por classe Pessoa
    ...
    def __str__(self): #para que a expressão seja avaliada
        return f'idade: {self.idade}, sexo: {self.sexo}, salario: {round(self.salario,2)}, intencao_de_voto {self.intencao_de_voto}' #devolvendo uma string
...
def rotulo_de_maior_frequencia (pessoas):
    ...
    mais_frequentes = frequencias.most_common(1) #most_commn devolve uma lista de tuplas
    return mais_frequentes[0][0]
    
def rotulo_de_maior_frequencia_sem_empate(pessoas): #rotulo para devolver o voto
    frequencias = Counter(pessoas)
